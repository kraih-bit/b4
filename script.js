...
// تحميل البيانات من GitHub Pages
fetch("data.json")
  .then(res => res.json())
  .then(json => {
    data = json;
  ...
  .catch(err => {
    alert("خطأ في تحميل البيانات! تأكد أن data.json موجود في نفس المستودع وفرع GitHub Pages.");
    console.error(err);
  });
    ...
    timer = setInterval(() => {
        if(index < data.length) {
            ...
            if(position) {
                ...
                profit = (position === "buy") ? price - entry : entry - price;
